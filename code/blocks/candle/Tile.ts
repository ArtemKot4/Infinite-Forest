class CandleTileReplacer {
  ...
  Saver.addSavesScope("scope.infinite_forest.candle_tile_replacer",
   function read(scope) {
    CandleTileReplacer.coordsList = scope?.coordsList || new Map<string, int>();
},
function save() {
    return { coordsList: CandleTileReplacer.coordsList };
});

namespace LevelDisplayedFunctions {
